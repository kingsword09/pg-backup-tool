...
async function main() {
  ...
  if (!platformId) {
    throw new Error(`Unsupported platform for local build: ${process.platform}-${process.arch}`);
  }
  ...
  if (!platformConfig) {
    throw new Error(`Could not find build configuration for platform: ${platformId}`);
  }
  ...
  if (process.platform === 'darwin' || process.platform === 'linux') {
    await runCommand('bash', [buildScriptPath], { env: { ...process.env, ...platformConfig } });
  ...
  const installDir = path.join(rootDir, 'node_modules', '.build', artifactDirName);
  ...
  await fs.cp(path.join(installDir, 'bin'), path.join(targetDir, 'bin'), { recursive: true });
  await fs.cp(path.join(installDir, 'lib'), path.join(targetDir, 'lib'), { recursive: true });
